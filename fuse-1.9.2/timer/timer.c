...
#else
  /* The native MorphOS AHI driver is an asynchronous producer/worker queue.
     It deliberately never blocks sound_lowlevel_frame(), so sound cannot be
     used as Fuse's pacing clock here. Always use the wall-clock timer path
     below; the AHI worker independently corrects small queue-depth drift. */
#endif
