...
menu_options_general_init( HWND hwndDlg )
{
  ...
  SendDlgItemMessage( hwndDlg, IDC_OPT_GENERAL_FULL_SCREEN, BM_SETCHECK,
    settings_current.full_screen ? BST_CHECKED : BST_UNCHECKED, 0 );
...
menu_options_general_done( HWND hwndDlg )
{
  ...
  settings_current.full_screen =
    IsDlgButtonChecked( hwndDlg, IDC_OPT_GENERAL_FULL_SCREEN );
...
static void
menu_options_peripherals_sound_init( HWND hwndDlg )
{
  char buffer[80];
  int i;
  ...
  SendDlgItemMessage( hwndDlg, IDC_OPT_PERIPHERALS_SOUND_FULLER, BM_SETCHECK,
    settings_current.fuller ? BST_CHECKED : BST_UNCHECKED, 0 );

  SendDlgItemMessage( hwndDlg, IDC_OPT_PERIPHERALS_SOUND_MELODIK, BM_SETCHECK,
    settings_current.melodik ? BST_CHECKED : BST_UNCHECKED, 0 );

  SendDlgItemMessage( hwndDlg, IDC_OPT_PERIPHERALS_SOUND_SPECDRUM, BM_SETCHECK,
    settings_current.specdrum ? BST_CHECKED : BST_UNCHECKED, 0 );

  SendDlgItemMessage( hwndDlg, IDC_OPT_PERIPHERALS_SOUND_COVOX, BM_SETCHECK,
    settings_current.covox ? BST_CHECKED : BST_UNCHECKED, 0 );

  SendDlgItemMessage( hwndDlg, IDC_OPT_PERIPHERALS_SOUND_USPEECH, BM_SETCHECK,
    settings_current.uspeech ? BST_CHECKED : BST_UNCHECKED, 0 );

}

static void
menu_options_peripherals_sound_done( HWND hwndDlg )
{
  char buffer[80];
  ...
  settings_current.fuller =
    IsDlgButtonChecked( hwndDlg, IDC_OPT_PERIPHERALS_SOUND_FULLER );

  settings_current.melodik =
    IsDlgButtonChecked( hwndDlg, IDC_OPT_PERIPHERALS_SOUND_MELODIK );

  settings_current.specdrum =
    IsDlgButtonChecked( hwndDlg, IDC_OPT_PERIPHERALS_SOUND_SPECDRUM );

  settings_current.covox =
    IsDlgButtonChecked( hwndDlg, IDC_OPT_PERIPHERALS_SOUND_COVOX );

  settings_current.uspeech =
    IsDlgButtonChecked( hwndDlg, IDC_OPT_PERIPHERALS_SOUND_USPEECH );
...
static BOOL CALLBACK
menu_options_peripherals_sound_proc( HWND hwndDlg, UINT msg, WPARAM wParam GCC_UNUSED,
                              LPARAM lParam GCC_UNUSED )
{
  switch( msg )
  {
    case WM_INITDIALOG:
    {
      ...
      /* initialize the controls with current settings */
      menu_options_peripherals_sound_init( hwndDlg );
    ...
    case WM_COMMAND:
      switch( LOWORD( wParam ) )
      {
        case IDOK:
          /* Read the controls and apply the settings */
          menu_options_peripherals_sound_done( hwndDlg );
          return 0;
...
void
menu_options_peripherals_sound( int action GCC_UNUSED )
{
  fuse_emulation_pause();

  DialogBox( fuse_hInstance, MAKEINTRESOURCE( IDD_OPT_PERIPHERALS_SOUND ),
             fuse_hWnd, (DLGPROC) menu_options_peripherals_sound_proc );
...
static const int sound_speaker_type_combo_count = 4;
...
menu_options_sound_init( HWND hwndDlg )
{
  ...
  SendDlgItemMessage( hwndDlg, IDC_OPT_SOUND_VOLUME_USPEECH, EM_LIMITTEXT,
                      3, 0 );
  /* FIXME This is asuming SendDlgItemMessage is not UNICODE */
  snprintf( buffer, 80, "%d", settings_current.volume_uspeech );
  SendDlgItemMessage( hwndDlg, IDC_OPT_SOUND_VOLUME_USPEECH, WM_SETTEXT,
                      0, (LPARAM) buffer );
...
menu_options_sound_done( HWND hwndDlg )
{
  ...
  /* FIXME This is asuming SendDlgItemMessage is not UNICODE */
  SendDlgItemMessage( hwndDlg, IDC_OPT_SOUND_VOLUME_USPEECH, WM_GETTEXT,
                      80, (LPARAM) buffer );
  settings_current.volume_uspeech = atoi( buffer );
