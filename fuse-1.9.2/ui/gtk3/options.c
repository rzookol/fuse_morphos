			 ...
			 gpointer data GCC_UNUSED )
{
  ...
  dialog.full_screen =
    gtk_check_button_new_with_label( "Full screen" );
  gtk_toggle_button_set_active( GTK_TOGGLE_BUTTON( dialog.full_screen ),
                                settings_current.full_screen );
  gtk_container_add( GTK_CONTAINER( content_area ), dialog.full_screen );
			      ...
			      gpointer user_data )
{
  ...
  settings_current.full_screen =
    gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( ptr->full_screen ) );
...
static void menu_options_peripherals_sound_done( GtkWidget *widget,
					  gpointer user_data );

void
menu_options_peripherals_sound( GtkWidget *widget GCC_UNUSED,
			 gpointer data GCC_UNUSED )
{
  GtkWidget *content_area;
  menu_options_peripherals_sound_t dialog;

  /* Firstly, stop emulation */
  fuse_emulation_pause();

  /* Create the necessary widgets */
  dialog.dialog = gtkstock_dialog_new( "Fuse - Sound Peripheral Options", NULL );
  content_area = gtk_dialog_get_content_area( GTK_DIALOG( dialog.dialog ) );
  ...
  dialog.uspeech =
    gtk_check_button_new_with_label( "uSpeech" );
  gtk_toggle_button_set_active( GTK_TOGGLE_BUTTON( dialog.uspeech ),
                                settings_current.uspeech );
  gtk_container_add( GTK_CONTAINER( content_area ), dialog.uspeech );

  /* Create the OK and Cancel buttons */
  gtkstock_create_ok_cancel( dialog.dialog, NULL,
                             G_CALLBACK( menu_options_peripherals_sound_done ),
                             (gpointer) &dialog, NULL, DEFAULT_DESTROY );
...
static void
menu_options_peripherals_sound_done( GtkWidget *widget GCC_UNUSED,
			      gpointer user_data )
{
  menu_options_peripherals_sound_t *ptr = user_data;
  ...
  settings_current.uspeech =
    gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( ptr->uspeech ) );
...
static const guint sound_speaker_type_combo_count = 4;
			 ...
			 gpointer data GCC_UNUSED )
{
  ...
  {
    GtkWidget *frame = gtk_frame_new( "uSpeech volume" );
    GtkWidget *hbox = gtk_box_new( GTK_ORIENTATION_HORIZONTAL, 0 );
    GtkWidget *text = gtk_label_new( "%" );
    gchar buffer[80];
    ...
    dialog.volume_uspeech = gtk_entry_new();
    gtk_entry_set_max_length( GTK_ENTRY( dialog.volume_uspeech ),
                              3 );
    snprintf( buffer, 80, "%d", settings_current.volume_uspeech );
    gtk_entry_set_text( GTK_ENTRY( dialog.volume_uspeech ), buffer );
    gtk_entry_set_activates_default( GTK_ENTRY( dialog.volume_uspeech ), TRUE );

    gtk_box_pack_start( GTK_BOX( hbox ), dialog.volume_uspeech, TRUE, TRUE, 0 );
			      ...
			      gpointer user_data )
{
  ...
  settings_current.volume_uspeech =
    atoi( gtk_entry_get_text( GTK_ENTRY( ptr->volume_uspeech ) ) );
