...
int widget_do( widget_type which, void *data )
{
...
#ifdef UI_MORPHOS
  if( ui_widget_level == -1 ) morphosui_dialog_begin();
#endif
  ...
  ui_widget_level--;
#ifdef UI_MORPHOS
  if( ui_widget_level == -1 ) morphosui_dialog_end();
#endif
...
{
#ifdef UI_MORPHOS
  return morphosui_statusbar_update( item, state );
#else
  (void)item; (void)state;
  return 0;
#endif
}
...
{
#ifdef UI_MORPHOS
  /* Keep the main MUI title stable ("Fuse for MorphOS").  The measured
     emulation speed belongs in the status bar; repeatedly changing the
     Window title makes it look like a runaway counter and causes needless
     MUI title updates. */
  morphosui_statusbar_speed( speed );
#else
  (void)speed;
#endif
  return 0;
