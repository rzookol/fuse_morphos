...
static int  widget_peripherals_sound_running = 0;
...
static void widget_uspeech_click( void );
static void widget_option_uspeech_draw( int left_edge, int width, struct widget_option_entry *menu, settings_info *show );
...
static void widget_volume_uspeech_click( void );
static void widget_option_volume_uspeech_draw( int left_edge, int width, struct widget_option_entry *menu, settings_info *show );
...
static widget_option_entry options_general[] = {
  ...
  { "Full \012s\001creen", 10, INPUT_KEY_s, NULL, NULL, widget_full_screen_click, widget_option_full_screen_draw },
...
static widget_option_entry options_peripherals_general[] = {
  ...
  { "\012I\001nterface 1", 3, INPUT_KEY_i, NULL, NULL, widget_interface1_click, widget_option_interface1_draw },
  { "In\012t\001erface 2", 4, INPUT_KEY_t, NULL, NULL, widget_interface2_click, widget_option_interface2_draw },
  { "Mu\012l\001tiface One", 5, INPUT_KEY_l, NULL, NULL, widget_multiface1_click, widget_option_multiface1_draw },
  { "Multiface 12\0128\001", 6, INPUT_KEY_8, NULL, NULL, widget_multiface128_click, widget_option_multiface128_draw },
  { "Multiface \0123\001", 7, INPUT_KEY_3, NULL, NULL, widget_multiface3_click, widget_option_multiface3_draw },
  { "Stealt\012h\001 Multiface One", 8, INPUT_KEY_h, NULL, NULL, widget_multiface1_stealth_click, widget_option_multiface1_stealth_draw },
  { "Emulate \012p\001rinters", 9, INPUT_KEY_p, NULL, NULL, widget_printer_click, widget_option_printer_draw },
  { "\012Z\001X Printer", 10, INPUT_KEY_z, NULL, NULL, widget_zxprinter_click, widget_option_zxprinter_draw },
  { "Speccy\012B\001oot interface", 11, INPUT_KEY_b, NULL, NULL, widget_speccyboot_click, widget_option_speccyboot_draw },
  { "Spectra\012n\001et", 12, INPUT_KEY_n, NULL, NULL, widget_spectranet_click, widget_option_spectranet_draw },
  { "Spe\012c\001tranet disable", 13, INPUT_KEY_c, NULL, NULL, widget_spectranet_disable_click, widget_option_spectranet_disable_draw },
...
static widget_option_entry options_peripherals_sound[] = {
  { "Sound Peripheral Options" },
  { "\012F\001uller Box", 0, INPUT_KEY_f, NULL, NULL, widget_fuller_click, widget_option_fuller_draw },
  { "M\012e\001lodik", 1, INPUT_KEY_e, NULL, NULL, widget_melodik_click, widget_option_melodik_draw },
  { "Spec\012D\001rum interface", 2, INPUT_KEY_d, NULL, NULL, widget_specdrum_click, widget_option_specdrum_draw },
  { "C\012o\001vox interface", 3, INPUT_KEY_o, NULL, NULL, widget_covox_click, widget_option_covox_draw },
  { "\012u\001Speech", 4, INPUT_KEY_u, NULL, NULL, widget_uspeech_click, widget_option_uspeech_draw },
  { NULL }
};
...
static widget_option_entry options_sound[] = {
  ...
  { "\012u\001Speech volume", 9, INPUT_KEY_u, "%", NULL, widget_volume_uspeech_click, widget_option_volume_uspeech_draw },
  { NULL }
};
...
int
widget_peripherals_sound_draw( void *data GCC_UNUSED )
{
  int error;

  if( !widget_peripherals_sound_running ) {		/* we want to copy settings, only when start up */
    highlight_line = 0;
    /* Get a copy of the current settings */
    settings_copy( &widget_options_settings, &settings_current );
    widget_peripherals_sound_running = 1;
  }

  error = widget_options_show_all( options_peripherals_sound, &widget_options_settings );
  if( error ) { settings_free( &widget_options_settings ); return error; }
...
static void
widget_uspeech_click( void )
{
  widget_options_settings.uspeech = ! widget_options_settings.uspeech;
}

static void
widget_option_uspeech_draw( int left_edge, int width, struct widget_option_entry *menu, settings_info *show )
{
  widget_options_print_option( left_edge, width, menu->index, menu->text, show->uspeech );
}

void
widget_peripherals_sound_keyhandler( input_key key )
{
  int new_highlight_line = 0;
  int cursor_pressed = 0;
  widget_option_entry *ptr;
  int menu_width = widget_calculate_option_width(options_peripherals_sound);
  int menu_left_edge_x = DISPLAY_WIDTH_COLS/2-menu_width/2;
  ...
  case INPUT_KEY_Resize:	/* Fake keypress used on window resize */
    ...
    widget_peripherals_sound_show_all( &widget_options_settings );
    break;
#endif
  ...
  case INPUT_JOYSTICK_FIRE_2:
    widget_end_widget( WIDGET_FINISHED_CANCEL );
    widget_peripherals_sound_running = 0;
    break;
  ...
  case INPUT_KEY_0:
  case INPUT_JOYSTICK_RIGHT:
    options_peripherals_sound[highlight_line+1].click();
    options_peripherals_sound[highlight_line+1].draw( menu_left_edge_x, menu_width, options_peripherals_sound + highlight_line + 1, &widget_options_settings );
    return;
  ...
  if( cursor_pressed ) {
    int old_highlight_line = highlight_line;
    highlight_line = new_highlight_line;
    options_peripherals_sound[old_highlight_line+1].draw( menu_left_edge_x, menu_width, options_peripherals_sound + old_highlight_line + 1, &widget_options_settings );
    options_peripherals_sound[highlight_line+1].draw( menu_left_edge_x, menu_width, options_peripherals_sound + highlight_line + 1, &widget_options_settings );
    return;
  }

  for( ptr=&options_peripherals_sound[1]; ptr->text != NULL; ptr++ ) {
    if( key == ptr->key ) {
      int old_highlight_line = highlight_line;
      ptr->click();
      highlight_line = ptr->index;
...
static void
widget_volume_uspeech_click( void )
{
  widget_text_t text_data;

  text_data.title = "uSpeech volume";
  text_data.allow = WIDGET_INPUT_DIGIT;
  text_data.max_length = 3;
  snprintf( text_data.text, 40, "%d",
            widget_options_settings.volume_uspeech );
  widget_do_text( &text_data );

  if( widget_text_text ) {
    widget_options_settings.volume_uspeech = atoi( widget_text_text );
  }
}

static void
widget_option_volume_uspeech_draw( int left_edge, int width, struct widget_option_entry *menu, settings_info *show )
{
  widget_options_print_entry( left_edge, width, menu->index, menu->text, show->volume_uspeech,
                              menu->suffix );
}
...
widget_sound_keyhandler( input_key key )
{
...
#if 0
  case INPUT_KEY_Resize:	/* Fake keypress used on window resize */
    widget_dialog_with_border( 1, 2, 30, 2 + 10 );
  ...
  case INPUT_KEY_End:
    if ( highlight_line + 2 < 10 ) {
      new_highlight_line = 10 - 1;
      cursor_pressed = 1;
    }
    break;
