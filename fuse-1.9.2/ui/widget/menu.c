...
{
  int result = set_active( widget_menu, path, active );
#ifdef UI_MORPHOS
  /* Keep the native MUI menu in sync with the widget menu state used by
     Fuse internals (RZX/AY/tape/media enable and disable transitions). */
  if( morphosui_menu_item_set_active( path, active ) == 0 ) result = 0;
#endif
  return result;
}
