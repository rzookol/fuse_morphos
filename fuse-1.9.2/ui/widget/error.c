...
ui_error_specific( ui_error_level severity, const char *message )
{
  widget_error_t error_info;
#ifdef UI_MORPHOS
  /* Serious MorphOS errors use a native requester instead of drawing a
     modal dialog into the emulated Spectrum framebuffer. */
  if( severity == UI_ERROR_ERROR && morphosui_error_request( severity, message ) )
    return 0;
#endif
