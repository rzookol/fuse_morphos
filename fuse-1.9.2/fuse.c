...
#include "ui/morphos/morphosui.h"
#endif
...
int fuse_exiting;
#ifdef __MORPHOS__
static int morphos_startup_manager_done;
#endif
...
#ifdef __MORPHOS__
/* Amiga-style version tag embedded in the executable for the Version command. */
const char morphos_verstag[] __attribute__((used)) =
  "\0$VER: Fuse " VERSION " (05.09.2026)";

/* Native MorphOS PPC stack size requested by libnix startup.
 * This symbol must be global (not static). */
int __stack = 100000;
#endif
...
#ifdef __MORPHOS__
  MOSDBG( "main enter argc=%ld __stack=%ld", (LONG)argc, (LONG)__stack );
#ifdef NO_SOUND
  MOSDBG( "audio compile-time state: NO_SOUND/null driver" );
#else
  MOSDBG( "audio compile-time state: enabled" );
#endif
  MOSDBG( "fuse_init begin" );
#endif
...
#ifdef __MORPHOS__
    MOSDBG( "fuse_init FAILED fuse_exiting=%ld startup_done=%ld",
            (LONG)fuse_exiting, (LONG)morphos_startup_manager_done );
    if( fuse_exiting && morphos_startup_manager_done ) {
      MOSDBG( "fuse_init aborted by user; running normal teardown" );
      fuse_end();
      MOSDBG( "early-exit teardown done" );
      return 0;
    }
#endif
#ifdef __MORPHOS__
    ui_error( UI_ERROR_ERROR, "Error initialising -- giving up" );
#else
...
#ifdef __MORPHOS__
    MOSDBG( "emulation loop leave fuse_exiting=%ld", (LONG)fuse_exiting );
#endif
    r = debugger_get_exit_code();
  }

#ifdef __MORPHOS__
  MOSDBG( "fuse_end begin" );
#endif
  fuse_end();
#ifdef __MORPHOS__
  MOSDBG( "fuse_end done rc=%ld", (LONG)r );
#endif
...
#ifdef __MORPHOS__
  MOSDBG( "fuse_init run_startup_manager begin" );
#endif
  if( run_startup_manager( &argc, &argv ) ) {
#ifdef __MORPHOS__
    MOSDBG( "fuse_init run_startup_manager FAILED exiting=%ld", (LONG)fuse_exiting );
#endif
    return 1;
  }
#ifdef __MORPHOS__
  morphos_startup_manager_done = 1;
  MOSDBG( "fuse_init run_startup_manager done" );
  /* machine_types[] is registered by the startup manager, while the MUI
     menustrip was created earlier from display_init()->ui_init().  Populate
     the native machine submenu only now, when the complete machine list is
     available. */
  morphosui_populate_machine_menu();
  /* ui_init() loaded ENV:/ENVARC:Fuse_MorphOS_prefs while the startup
     manager ran. Refresh this local copy so the remembered native filter is
     what the normal Fuse scaler selection below actually applies. */
  if( settings_current.start_scaler_mode ) {
    libspectrum_free( start_scaler );
    start_scaler = utils_safe_strdup( settings_current.start_scaler_mode );
  }
  MOSDBG( "fuse_init prefs scaler=%s", start_scaler ? start_scaler : "(null)" );
  MOSDBG( "fuse_init machine_select begin id=%ld", (LONG)settings_current.start_machine );
#endif

  error = machine_select_id( settings_current.start_machine );
#ifdef __MORPHOS__
  MOSDBG( "fuse_init machine_select done error=%ld exiting=%ld", (LONG)error, (LONG)fuse_exiting );
#endif
  if( error ) return error;

#ifdef __MORPHOS__
  MOSDBG( "fuse_init scaler_select begin name=%s", start_scaler ? start_scaler : "(null)" );
#endif
  error = scaler_select_id( start_scaler ); libspectrum_free( start_scaler );
#ifdef __MORPHOS__
  MOSDBG( "fuse_init scaler_select done error=%ld exiting=%ld", (LONG)error, (LONG)fuse_exiting );
#endif
  if( error ) return error;
...
{
#ifdef __MORPHOS__
  MOSDBG( "fuse_end movie_stop begin" );
#endif
  movie_stop();		/* stop movie recording */
#ifdef __MORPHOS__
  MOSDBG( "fuse_end movie_stop done" );
  MOSDBG( "fuse_end startup_manager begin" );
#endif
  startup_manager_run_end();
#ifdef __MORPHOS__
  MOSDBG( "fuse_end startup_manager done" );
  MOSDBG( "fuse_end periph_end begin" );
#endif
  periph_end();
#ifdef __MORPHOS__
  MOSDBG( "fuse_end periph_end done" );
  MOSDBG( "fuse_end ui_end begin" );
#endif
  ui_end();
#ifdef __MORPHOS__
  MOSDBG( "fuse_end ui_end done" );
  MOSDBG( "fuse_end ui_media_drive_end begin" );
#endif
  ui_media_drive_end();
#ifdef __MORPHOS__
  MOSDBG( "fuse_end ui_media_drive_end done" );
  MOSDBG( "fuse_end module_end begin" );
#endif
  module_end();
#ifdef __MORPHOS__
  MOSDBG( "fuse_end module_end done" );
  MOSDBG( "fuse_end pokemem_end begin" );
#endif
  pokemem_end();
#ifdef __MORPHOS__
  MOSDBG( "fuse_end pokemem_end done" );
  MOSDBG( "fuse_end svg_capture_end begin" );
#endif
  svg_capture_end();
#ifdef __MORPHOS__
  MOSDBG( "fuse_end svg_capture_end done" );
  MOSDBG( "fuse_end libspectrum_end begin" );
#endif
  libspectrum_end();
#ifdef __MORPHOS__
  MOSDBG( "fuse_end libspectrum_end done" );
#endif
  return 0;
}
